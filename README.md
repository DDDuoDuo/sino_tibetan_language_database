<div align="center">

<img src="https://www.eduhk.hk/re/modules/content/images/logo_eduhk.png" height="80" alt="EdUHK Logo" />

# 汉藏语分化时间数据库
### Sino-Tibetan Divergence Time Database

[![Generic badge](https://img.shields.io/badge/Subject-Historical_Linguistics-blue.svg)](http://47.238.241.178/index.html)
[![Generic badge](https://img.shields.io/badge/Status-Active-success.svg)](http://47.238.241.178/index.html)
[![Generic badge](https://img.shields.io/badge/Institution-EdUHK-0056b3.svg)](https://www.eduhk.hk/)

[**访问数据库 (Enter Database)**](http://47.238.241.178/index.html)

</div>

<br>

## 项目简介 | Introduction

汉藏语分化时间数据库旨在通过历史比较法，基于词汇证据，收集汉藏语系内各语言与方言的分化时间信息。

这些词汇证据自带时间信息，如工具材料名称（石、铁、铜）、驯化动物、外来物种或概念、借词等。结合考古研究结果和历史文献资料，通过将词汇进行**历史分层（Lexical Stratigraphic Analysis）**，可以判定目标语言或方言的分化上下限。用户可通过本数据库追溯语言分化历程，并比较语系内不同分支的分化模式。

---

## 数据与贡献 | Contribution

本数据库所收录的词汇与语言数量将持续增加，并随着更多研究者的参与而不断丰富。每个数据集均由相应语言及其历史演变领域的专家提供。

### 引用规范 (Citation)

在使用数据库资料时，请注明相关贡献者及本项目出处。数据库支持按语言、时间段、词汇或贡献者等多种方式进行检索与访问。

> **引用格式：**
>
> 贡献者姓名、年份、项目名、website name, accessed on Year-Month-Date.

---

## 团队介绍 | Team

该数据库旨致力于为语言学和历史研究者提供一个集科研、资源共享与学术交流于一体的专业平台。

| 角色 | 姓名 | 所属机构 |
| :--- | :--- | :--- |
| **设计** | **丁泓棣 博士** | 香港教育大学 人文学院中国语言及中文教育研究中心 |
| **设计** | **梁源 博士** | 香港教育大学 人文学院中国语言及中文教育研究中心 |
| **开发与维护** | **梁仁隽** | 独立学者 |

<br>

## 联系方式 | Contact

如有任何问题或建议，欢迎联系：
**hongdiding@gmail.com**
